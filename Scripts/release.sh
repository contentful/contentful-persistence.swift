#!/bin/bash
# Release contentful-persistence.swift. CircleCI runs the same steps as a manual release.
#
...
#   xcframework     build Carthage/Build/ContentfulPersistence.xcframework (library evolution on) and
#                   zip it (ditto) into build/release/
...
REPO_SLUG="contentful/contentful-persistence.swift"
GIT_REMOTE="${GIT_REMOTE:-origin}"
RELEASE_DIR="$ROOT/build/release"
FRAMEWORK="ContentfulPersistence"
ZIP_NAME="$FRAMEWORK.xcframework.zip"
DRY_RUN="${DRY_RUN:-0}"

VERSION_KEY="CONTENTFUL_PERSISTENCE_VERSION"
VERSION="$(sed -n "s/^$VERSION_KEY=//p" .env | tr -d '[:space:]')"
...
step_validate() {
  ...
  xcconfig_version="$(sed -n "s/^$VERSION_KEY=//p" Config.xcconfig | tr -d '[:space:]')"
...
# The runtime dependency (contentful.swift) must be built before ContentfulPersistence can link against it.
bootstrap_contentful() {
  require carthage "Install it with: brew install carthage"
  if [[ ! -d Carthage/Build/Contentful.xcframework || ! -d Carthage/Checkouts/contentful.swift/Contentful.xcodeproj ]]; then
    log "Building the contentful.swift dependency pinned in Cartfile.resolved"
    carthage bootstrap contentful.swift --use-xcframeworks --derived-data "$ROOT/build/DerivedData"
  fi
}

step_xcframework() {
  log "Building $FRAMEWORK.xcframework $VERSION"
  require ditto "ditto ships with macOS"
  bootstrap_contentful

  rm -rf "Carthage/Build/$FRAMEWORK.xcframework" "$RELEASE_DIR"
  mkdir -p "$RELEASE_DIR"

  # Library evolution makes the prebuilt binary usable with newer Swift compilers, not only the one
  # it was built with. It is enabled for the release build only; the Xcode project keeps its settings.
  local xcconfig="$ROOT/build/release.xcconfig"
  echo "BUILD_LIBRARY_FOR_DISTRIBUTION = YES" > "$xcconfig"

  XCODE_XCCONFIG_FILE="$xcconfig" carthage build "$FRAMEWORK" \
    --no-skip-current \
    --platform all \
    --use-xcframeworks \
    --derived-data "$ROOT/build/DerivedData"

  [[ -d "Carthage/Build/$FRAMEWORK.xcframework" ]] || fail "Carthage did not produce Carthage/Build/$FRAMEWORK.xcframework"

  # ditto keeps the symlinks inside the macOS framework slice. zip -r would break them.
  ditto -c -k --sequesterRsrc --keepParent "Carthage/Build/$FRAMEWORK.xcframework" "$RELEASE_DIR/$ZIP_NAME"
...
step_docs() {
  ...
  # ContentfulPersistence.xcworkspace builds against Carthage/Checkouts/contentful.swift (a submodule a new
  # worktree leaves empty) and Carthage/Build (git-ignored). Reuse the ones from this checkout.
  rm -rf "$worktree/Carthage"
  ln -s "$ROOT/Carthage" "$worktree/Carthage"
...
case "${1:-}" in
  ...
  all)
    ...
    if [[ "$DRY_RUN" == "1" ]]; then
      ...
      log "ContentfulPersistence $VERSION released"
    fi
    ;;
  *)
