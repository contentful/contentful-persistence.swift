...
PROJECT_SLUG="gh/contentful/contentful-persistence.swift"

: "${CIRCLE_TOKEN:?Set CIRCLE_TOKEN to a CircleCI personal API token (https://app.circleci.com/settings/user/tokens)}"

git fetch --quiet origin master
VERSION="$(git show origin/master:.env | sed -n 's/^CONTENTFUL_PERSISTENCE_VERSION=//p' | tr -d '[:space:]')"
...
echo "About to release contentful-persistence.swift $VERSION from origin/master ($COMMIT)."
if [[ "${1:-}" != "--yes" ]]; then
